  ...
  (let* ((var (window-predicate-var predicate))
    ...
    (cond
      ...
      ((setq cmd (window-predicate-cmd predicate))
       (y-or-no-p (format nil "No window that satisfies predicate ~A.~%Do you want to run ~A?" (window-predicate-name) cmd))))))
...
(define-window-predicate emacs (w)
  (classed-p w "Emacs"))
(define-window-predicate chrome (w)
  (classed-p w "Google-chrome"))
(define-window-predicate firefox (w)
  (classed-p w "Firefox"))
(define-window-predicate konsole (w)
  (classed-p w "konsole"))
(define-window-predicate krusader (w)
  (classed-p w "Krusader"))
(define-window-predicate wireshark (w)
